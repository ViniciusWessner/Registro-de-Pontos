...
GerarRelatorio() {
    local data=$(dialog --inputbox "Digite a data para o relatório (YYYY-MM-DD) ou deixe em branco para todas as datas:" 8 50 3>&1 1>&2 2>&3)
    
    if [ -z "$data" ]; then
        relatorio=$(cat "$ARQUIVO_REGISTRO")
    else
        relatorio=$(grep "^$data$SEP" "$ARQUIVO_REGISTRO")
        [ -z "$relatorio" ] && relatorio="Nenhum registro encontrado para a data especificada."
    fi
    
    dialog --title "Relatório de Ponto" --msgbox "$relatorio" 20 50
}
